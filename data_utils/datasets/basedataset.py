...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        # if the training/testing include noisy setting wild参数设置
        ...
        # first load all paths of different scenarios 收集所有场景文件的路径
        ...
        # loop over all scenarios #得到所有场景的文件
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            # be ego vehicle.#路侧单元的数据ID常常是负值，所以我们需要在列表的终点，而且不能作为主要路径
            if int(cav_list[0]) < 0:
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        # check the timestamp index  检查时间戳索引
        timestamp_index = idx if scenario_index == 0 else \
            idx - self.len_record[scenario_index - 1]
        # retrieve the corresponding timestamp key 获取对应的时间戳位置
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # calculate delay for this vehicle 计算延时
            timestamp_delay = \
            ...
            # add time delay to vehicle parameters 增加时延
            data[cav_id]['time_delay'] = timestamp_delay
            # load the corresponding data into the dictionary 加载其他参数
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        # Find ego pose first 首先寻找到中心车辆位置
        for cav_id, cav_content in scenario_database.items():
            if cav_content['ego']:
