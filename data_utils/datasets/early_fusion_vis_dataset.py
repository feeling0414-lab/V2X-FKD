...
class EarlyFusionVisDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        # first find the ego vehicle's lidar pose 得到中间车辆的点云雷达位置
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        # retrieve objects under ego coordinates 在自我坐标下检索对象
        ...
        lidar_np, object_bbx_center, mask = \
             self.augment(lidar_np, object_bbx_center, object_bbx_mask)

        lidar_np = shuffle_points(lidar_np)
        # remove points that hit itself 移除击中自身的点
        lidar_np = mask_ego_points(lidar_np)
        # project the lidar to ego space 使用变换矩阵将激光数据投射到自身的空间
        lidar_np[:, :3] = \
