...
class CombineDatasetAugmentor(basedataset.BaseDataset):
    def __init__(self, params, visualize, train=True):
        super(CombineDatasetAugmentor, self). \
    ...
    def __getitem__(self, idx):
        ...
        projected_lidar_stack_early = []
        object_stack_early = []
        object_id_stack_early = []
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            # 前融合数据
            projected_lidar_stack_early.append(
                selected_cav_processed['projected_lidar'])
            object_stack_early.append(selected_cav_processed['object_bbx_center'])
            object_id_stack_early += selected_cav_processed['object_ids']
        ...
        unique_indices = \
            [object_id_stack_early.index(x) for x in set(object_id_stack_early)]
        object_stack = np.vstack(object_stack_early)
        ...
        lidar_len = []
        for i in range(len(projected_lidar_stack_early)):
            lidar_len.append(projected_lidar_stack_early[i].shape[0])

        # convert list to numpy array, (N, 4) 转化为numpy格式
        projected_lidar_stack = np.vstack(projected_lidar_stack_early)

        # data augmentation 数据增强
        projected_lidar_stack_aug, object_bbx_center_aug, mask_aug = \
            self.augment(projected_lidar_stack, object_bbx_center, mask)

        # we do lidar filtering in the stacked lidar 数据清洗
        projected_lidar_stack_early = mask_points_by_range(projected_lidar_stack_aug,
        ...
        mask_aug[object_bbx_center_valid.shape[0]:] = 0
        ...
        lidar_dict = self.pre_processor.preprocess(projected_lidar_stack_early)
        ...
        processed_data_dict_early['ego'].update(
            {'object_bbx_center': object_bbx_center,
             'object_bbx_mask': mask_aug,
             'object_ids': [object_id_stack_early[i] for i in unique_indices],
             'anchor_box': anchor_box,
             'processed_lidar': lidar_dict,
             'label_dict': label_dict,
             'origin_lidar': projected_lidar_stack_early})


        #intermediate部分
        processed_data_dict_intermediate = OrderedDict()
        processed_data_dict_intermediate['ego'] = {}
        processed_features_compare = []

        len_array = np.cumsum(lidar_len)
        for i in range(len(len_array)):
            if i==0:
                projected_lidar_stack_input = projected_lidar_stack[:len_array[i]]
            else:
                projected_lidar_stack_input = projected_lidar_stack[len_array[i-1]:len_array[i]]

            projected_lidar_stack_input = mask_points_by_range(projected_lidar_stack_input,
                                                         self.params['preprocess'][
                                                             'cav_lidar_range'])

            processed_dict = self.pre_processor.preprocess(projected_lidar_stack_input)
            processed_features_compare.append(processed_dict)

        cav_num = len(processed_features_compare)
        merged_feature_dict = self.merge_features_to_dict(processed_features_compare)
